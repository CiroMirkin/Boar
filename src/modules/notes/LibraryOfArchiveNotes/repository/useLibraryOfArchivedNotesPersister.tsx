...
export const useLibraryOfArchivedNotesPersister = () => {
	...
	const persistNotes = async (session: SessionType, notes: LibraryOfArchivedNotes) => {
		...
		const areNotDefaultNotes = JSON.stringify(notes.archive) !== JSON.stringify(defaultLibraryOfArchivedNotes.archive)
		...
		if (session) {
			...
			const areNotLocalNotes = JSON.stringify(localNotes.archive) !== JSON.stringify(notes.archive)
